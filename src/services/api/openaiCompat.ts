...
export function joinBaseUrl(baseURL: string, path: string): string {
  ...
  // For OpenAI chat completions path
  if (normalizedPath === 'v1/chat/completions') {
    // If baseURL already ends with /v1, just add /chat/completions
    if (normalizedBaseUrl.endsWith('/v1')) {
      return `${normalizedBaseUrl}/chat/completions`
    }
    // If baseURL already ends with /v1/chat/completions, return as is
...
export function convertAnthropicRequestToOpenAI(input: {
  ...
  // Validate and clamp max_tokens based on model limits
  let max_tokens = input.max_tokens
  if (max_tokens !== undefined) {
    // Get limits from global config or use defaults
    const globalConfig = getGlobalConfig()
    const modelLimits = globalConfig.apiLimits?.[targetModel.toLowerCase()] || 
                      globalConfig.apiLimits?.default || {
                        maxTokens: { min: 1, max: 8192 }
                      }
    
    if (max_tokens< modelLimits.maxTokens.min) {
      max_tokens = modelLimits.maxTokens.min
    } else if (max_tokens >modelLimits.maxTokens.max) {
      max_tokens = modelLimits.maxTokens.max
    }
  }
