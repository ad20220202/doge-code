...
export function ModelPicker(t0) {
  ...
  const t16 = headerText ?? "在不同模型间切换。适用于当前会话和未来的 Doge Code 会话。对于其他/先前的模型名称，请使用 --model 参数指定。";
  let t17;
  if ($[42] !== t16) {
  ...
  let t18;
  if ($[44] !== sessionModel) {
    t18 = sessionModel && <Text dimColor={true}>当前会话正在使用 {modelDisplayString(sessionModel)}（由计划模式设置）。选择模型将取消此设置。</Text>;
    $[44] = sessionModel;
    $[45] = t18;
  } else {
    t18 = $[45];
  ...
  let t22;
  if ($[57] !== hiddenCount) {
    t22 = hiddenCount > 0 && <Box paddingLeft={3}><Text dimColor={true}>以及 {hiddenCount} 个更多…</Text></Box>;
    $[57] = hiddenCount;
    $[58] = t22;
  } else {
    t22 = $[58];
  ...
  if ($[62] !== displayEffort || $[63] !== focusedDefaultEffort || $[64] !== focusedModelName || $[65] !== focusedSupportsEffort) {
    t24 = <Box marginBottom={1} flexDirection="column">{focusedSupportsEffort ? <Text dimColor={true}><EffortLevelIndicator effort={displayEffort} />{" "}{capitalize(displayEffort)} 努力程度{displayEffort === focusedDefaultEffort ? " (默认)" : ""}{" "}<Text color="subtle">← → 调整</Text></Text> : <Text color="subtle"><EffortLevelIndicator effort={undefined} /> 不支持努力程度{focusedModelName ? ` (${focusedModelName})` : ""}</Text>}</Box>;
    $[62] = displayEffort;
  ...
  if ($[67] !== showFastModeNotice) {
    t25 = isFastModeEnabled() ? showFastModeNotice ? <Box marginBottom={1}><Text dimColor={true}>快速模式已<Text bold={true}>开启</Text>，仅适用于{" "}{FAST_MODE_MODEL_DISPLAY}（使用 /fast 命令）。切换到其他模型将关闭快速模式。</Text></Box> : isFastModeAvailable() && !isFastModeCooldown() ? <Box marginBottom={1}><Text dimColor={true}>使用 <Text bold={true}>/fast</Text> 开启快速模式（仅适用于 {FAST_MODE_MODEL_DISPLAY}）。</Text></Box> : null : null;
