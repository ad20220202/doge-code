...
export function isAnthropicAuthEnabled(): boolean {
  ...
  // Check if user has configured custom API endpoint
  // If custom API endpoint is configured, disable Anthropic auth by default
  const customApiConfig = readCustomApiStorage()
  const hasCustomApiEndpoint = customApiConfig.provider && customApiConfig.baseURL

  // If custom API endpoint is configured, prioritize it over other auth methods
  if (hasCustomApiEndpoint) {
    // Allow env var to force OAuth if needed for complex scenarios
    return isEnvTruthy(process.env.CLAUDE_CODE_FORCE_OAUTH)
  }
