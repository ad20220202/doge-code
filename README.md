...
[![Config](https://img.shields.io/badge/config-~%2F.claude-8b5cf6)](README.md)
...
[`Claude Code`](README.md) 基于一份还原后的 [`Claude Code`](README.md) 源码树继续修改而来。
...
- 支持自定义 API Key
- 支持自定义模型与模型列表管理
- 尽量把自定义接入数据收口到 [`~/.claude`](README.md) 路径体系
- 在保留 CLI/TUI 主体结构的前提下，降低对官方登录流的绑定

那么，它现在更像一个"可自托管 / 可代理 / 可转接"的 Claude Code 变体。

## 与原版 Claude Code 的数据隔离

Claude Code 默认**不应**与原版 [`Claude Code`](README.md) 共用配置和缓存目录。

当前 Fork 已明确把默认用户目录收口到：

- 配置目录：[`~/.claude`](README.md)
- 全局配置文件：[`~/.claude/.claude.json`](README.md)

这样做的目的，是避免以下问题：

- 原版 [`Claude Code`](README.md) 的登录态污染本项目
- 原版保存的 endpoint / token / model 配置影响代理转接逻辑
- 两边共用 [`.claude.json`](README.md) 或 [`.claude/`](README.md) 导致奇怪的网络、认证、模型或 UI 异常

如果用户以前装过原版 [`Claude Code`](README.md)，再运行本项目时出现"明明没这么配却读到了旧配置"的现象，通常就是历史数据混用导致的。

建议：

- 原版继续使用它自己的 [`.claude`](README.md) / [`.claude.json`](README.md)
- 本项目使用 [`.claude`](README.md) 目录
- 如需手动指定，也可以通过 [`CLAUDE_CONFIG_DIR`](README.md) 指向独立目录
...
本项目正在加入一个"中间转接层"模式，用来让内部仍按 Anthropic Messages 结构工作的主逻辑，转发到 OpenAI Chat Completions 接口。
...
- 项目已被继续改造成 Claude Code 分支，部分行为和 UI 已不再与原始 Claude Code 一致
...
- 全局包名是 [`@anthropic-ai/claude-code`](package.json:2)
- 命令名是 [`claude`](package.json:24)
...
```bash
bun link @anthropic-ai/claude-code
```
...
{
  "dependencies": {
    "@anthropic-ai/claude-code": "link:@anthropic-ai/claude-code"
  }
}
```
...
也就是说，这个 Fork 现在使用的入口名是 [`claude`](README.md)。

如果你使用 [`bun link`](README.md) 进行全局注册链接，那么现在注册出来的包名是：
